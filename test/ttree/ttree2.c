...
const char* test_title (void)
   {
   return "ttree2: Tree Iterator Tests";
   }
...
int test_run (void)
   ...
   printf ("Replace tree via iterator.\n");

   iterator = tree_at (tree, 2);
   data = tree_data_get (iterator);
   printf (
          "\"%s\" = tree_data_get (iterator) ............. %s\n",
          data, ((strcmp(data,g_data1[0])==0) ? "PASS" : "FAIL")
          );
   tree_data_set (iterator, g_data2[0]);

   iterator = tree_at (tree, 1);
   data = tree_data_get (iterator);
   printf (
          "\"%s\" = tree_data_get (iterator) ............ %s\n",
          data, ((strcmp(data,g_data1[1])==0) ? "PASS" : "FAIL")
          );
   tree_data_set (iterator, g_data2[1]);

   iterator = tree_at (tree, 4);
   data = tree_data_get (iterator);
   printf (
          "\"%s\" = tree_data_get (iterator) ............. %s\n",
          data, ((strcmp(data,g_data1[2])==0) ? "PASS" : "FAIL")
          );
   tree_data_set (iterator, g_data2[2]);

   iterator = tree_at (tree, 3);
   data = tree_data_get (iterator);
   printf (
          "\"%s\" = tree_data_get (iterator) ............ %s\n",
          data, ((strcmp(data,g_data1[3])==0) ? "PASS" : "FAIL")
          );
   tree_data_set (iterator, g_data2[3]);

   for (i=0 ; i<4 ; i++) g_key[i] = 0;
   g_idx = 0;
   printf ("Foreach PREFIX on tree .............................. ");
   fflush (stdout);
   tree_foreach (tree, SCL_PREFIX, cbfn, NULL);
   ...
   fflush (stdout);
   tree_foreach (tree, SCL_INFIX, cbfn, NULL);
   ...
   tree_foreach (tree, SCL_POSTFIX, cbfn, NULL);
   fflush (stdout);
