...
const char* test_title (void)
   {
   return "tqueuebuf2: Queuebuf Peek, Get_Alloc Tests";
   }
...
int test_run (void)
   ...
   printf ("Peek at queuebuf while pulling data.\n");
   stat = queuebuf_peek (q, (void**)&g_dataPtr, &size);
   printf (
          "[%d,%s] = queuebuf_peek (q, &g_dataPtr, &size);\n",
          stat, scl_statstr(stat)
          );
   printf ("data(%ld) = \"%s\"(%d)\n", size, g_dataPtr, strlen(g_dataPtr)+1);
   stat = queuebuf_get_alloc (q, (void**)&g_dataPtr, &size);
   printf (
          "[%d,%s] = queuebuf_get_alloc (q, &g_dataPtr, &size);\n",
          stat, scl_statstr(stat)
          );
   printf ("data(%ld) = \"%s\"(%d)\n", size, g_dataPtr, strlen(g_dataPtr)+1);
   (void)cbfn (NULL, size, g_dataPtr, NULL);
   SCL_DEALLOCATOR(g_dataPtr);
